...
const StudentPortal = memo(function StudentPortal({ userInfo, onLogout }: StudentPortalProps) {
  ...
  const fetchStudentData = async () => {
    ...
    try {
      ...
      const [{ data: student }, { data: gradesData }, { data: attendanceData }, { data: homeworkData }, { data: feesData }, { data: termsData }] = await Promise.all([
        supabase.from('students').select('id, first_name, middle_name, last_name, email, login_id, student_id, class_id, parent_id, is_first_login, classes(class_name)').eq('id', studentId).single(),
        ...
        supabase.from('attendance').select('id, student_id, date, status').eq('student_id', studentId).order('date', { ascending: false }).limit(30),
        supabase.from('homeworks').select('id, title, description, due_date, created_at').order('created_at', { ascending: false }),
        supabase.from('payments').select('id, student_id, fee_type, amount, due_date, status, paid_at, created_at').eq('student_id', studentId).order('created_at', { ascending: false }),
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0f172a', color: '#f8fafc' }}>
                  ...
                  {homework.map((hw: any) => (
                    <div key={hw.id} style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 12, padding: 20 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <span style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, background: new Date(hw.due_date) < new Date() ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)', color: new Date(hw.due_date) < new Date() ? '#ef4444' : '#f59e0b' }}>
                    ...
                    <tbody>
                      {fees.map((f: any) => (
                        <tr key={f.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                          ...
                          <td style={{ padding: 14, textAlign: 'center' }}>
                            <span style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, background: f.status === 'paid' ? 'rgba(16,185,129,0.15)' : f.status === 'overdue' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)', color: f.status === 'paid' ? '#10b981' : f.status === 'overdue' ? '#ef4444' : '#f59e0b' }}>
                              {f.status}
                            </span>
                          </td>
                    ...
                    return (
                      <div key={term.id} style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                        <div>
                          ...
                          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>{term.is_active ? '🟢 Current Term' : '⚪ Past Term'}</p>
                        </div>
      ...
      {showPasswordModal && (
        <PasswordChangeModal userInfo={userInfo} userRole="student" onClose={() => setShowPasswordModal(false)}
