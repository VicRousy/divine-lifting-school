...
function StudentProfile({ student, onBack }: StudentProfileProps) {
  const [academicSquad, setAcademicSquad] = useState<any[]>([])
  ...
  const handlePrintReport = () => {
    ...
    const totalObtained = reportScores.reduce((acc: number, s: any) => acc + s.total_score, 0);
  ...
  const fetchData = async () => {
    ...
    const { data: fees } = await safeQuery(() => supabase
      .from('fees')
      .select('id, student_id, amount_paid, payment_method, term, payment_date')
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '400px', width: 'min(400px, calc(100vw - 32px))' }}>
            ...
            <div className="action-group" style={{ marginTop: '25px' }}>
              ...
              <button className="btn-edit" onClick={handleAddPayment}>Save Payment</button>
            </div>
          </div>
        </div>
      )}
