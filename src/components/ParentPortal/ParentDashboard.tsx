...
const ParentDashboard = memo(function ParentDashboard({ userInfo, onLogout }: ParentDashboardProps) {
  const [activeTab, setActiveTab] = useState('overview')
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const [showPasswordModal, setShowPasswordModal] = useState(false)
  const handleNavClick = (key: string) => { setActiveTab(key); setMobileMenuOpen(false) }
  ...
  const fetchChildData = async (childId: number) => {
    ...
    try {
      const [{ data: gradesData }, { data: attendanceData }, { data: feesData }, { data: homeworkData }, { data: termsData }] = await Promise.all([
        ...
        supabase.from('attendance').select('id, student_id, date, status').eq('student_id', childId).order('date', { ascending: false }).limit(30),
        supabase.from('payments').select('id, student_id, fee_type, amount, due_date, status, paid_at, created_at').eq('student_id', childId).order('created_at', { ascending: false }),
        supabase.from('homeworks').select('id, title, description, due_date, class_id, created_at').order('created_at', { ascending: false }),
  ...
  const handleDownloadReportCard = async (term: any) => {
    ...
    let totalScore = 0
    const subjects = termGrades.map((s: any) => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0f172a', color: '#f8fafc' }}>
      ...
      <aside className={`sidebar ${mobileMenuOpen ? 'open' : ''}`} style={{ width: 260, background: '#1e293b', borderRight: '1px solid #334155', display: 'flex', flexDirection: 'column', padding: '20px 0', flexShrink: 0 }}>
        {mobileMenuOpen && <div className="mobile-close-btn" style={{ textAlign: 'right', padding: '0 20px 10px' }}>
          <button onClick={() => setMobileMenuOpen(false)} aria-label="Close menu" style={{ background: 'none', border: 'none', color: '#f8fafc', fontSize: '1.5rem', cursor: 'pointer' }}>✕</button>
        </div>}
        ...
        <nav style={{ flex: 1, padding: '20px 0' }}>
          {[
            ...
            <div key={item.key} role="button" tabIndex={0} onClick={() => handleNavClick(item.key)}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleNavClick(item.key) }}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 20px', cursor: 'pointer', background: activeTab === item.key ? '#334155' : 'transparent', borderLeft: activeTab === item.key ? '3px solid #a855f7' : '3px solid transparent', color: activeTab === item.key ? '#f8fafc' : '#94a3b8', transition: 'all 0.2s' }}>
              <span>{item.icon}</span>
      ...
      <main className="main-layout" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <header style={{ background: '#1e293b', padding: '15px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #334155' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button className="hamburger-btn" onClick={() => setMobileMenuOpen(true)} aria-label="Open menu" style={{ background: 'none', border: 'none', color: '#f8fafc', fontSize: '1.5rem', cursor: 'pointer' }}>☰</button>
            <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            {children.length > 1 && (
              <select value={selectedChild?.id || ''} onChange={(e) => setSelectedChild(children.find((c: any) => String(c.id) === e.target.value))}
            ...
            <>
                        ...
                        {grades.slice(0, 5).map((g: any) => {
                          ...
                          return (
                            <div key={g.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'rgba(15, 23, 42, 0.5)', borderRadius: 8 }}>
                              <div><div style={{ fontWeight: 600, color: '#e2e8f0' }}>{g.subjects?.subject_name}</div><div style={{ fontSize: '0.85rem', color: '#94a3b8' }}>CA1: {g.ca1_score} | CA2: {g.ca2_score} | Exam: {g.exam_score}</div></div>
                              <div style={{ textAlign: 'right' }}><div style={{ fontWeight: 700, color: gradeInfo.color }}>{gradeInfo.grade}</div><div style={{ fontSize: '0.85rem', color: '#e2e8f0' }}>{total}/100</div></div>
                            </div>
                          )
                        })}
                      </div>
              ...
              {activeTab === 'grades' && (
                <div>
                  ...
                  {grades.length === 0 ? (
                    ...
                    <div className="responsive-table-wrap" style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 14 }}>
                          ...
                          {grades.map((g: any) => {
                            ...
                            return (
                              <tr key={g.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                ...
                                <td style={{ padding: 14, textAlign: 'center' }}><span style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.8rem', background: 'rgba(16,185,129,0.15)', color: '#10b981' }}>Approved</span></td>
                              </tr>
                            )
                          })}
                        </tbody>
              ...
              {activeTab === 'attendance' && (
                <div>
                  ...
                  <div className="portal-stats" style={{ display: 'flex', gap: 20, marginBottom: 20 }}>
                  ...
                  {attendance.length === 0 ? (
                    ...
                    <div className="responsive-table-wrap" style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 14 }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead><tr style={{ borderBottom: '1px solid #334155' }}>{['DATE', 'STATUS'].map((h) => (<th scope="col" key={h} style={{ padding: 14, textAlign: 'center', color: '#94a3b8', fontSize: '0.8rem' }}>{h}</th>))}</tr></thead>
                        <tbody>
                          {attendance.map((a: any) => (
                            <tr key={a.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                              ...
                              <td style={{ padding: 14, textAlign: 'center' }}>
                                <span style={{ padding: '4px 12px', borderRadius: 6, fontSize: '0.85rem', fontWeight: 600, background: a.status === 'present' ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)', color: a.status === 'present' ? '#10b981' : '#ef4444' }}>
              ...
              {activeTab === 'fees' && (
                <div>
                  ...
                  <div className="portal-stats" style={{ display: 'flex', gap: 20, marginBottom: 20 }}>
                    <div style={{ flex: 1, background: 'rgba(239,68,68,0.1)', border: '1px solid #ef4444', borderRadius: 10, padding: '16px 20px' }}><div style={{ fontSize: '0.85rem', color: '#ef4444', marginBottom: 4 }}>Total Pending</div><div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#ef4444' }}>₦{pendingFees.toLocaleString()}</div></div>
                    <div style={{ flex: 1, background: 'rgba(16,185,129,0.1)', border: '1px solid #10b981', borderRadius: 10, padding: '16px 20px' }}><div style={{ fontSize: '0.85rem', color: '#10b981', marginBottom: 4 }}>Total Paid</div><div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#10b981' }}>₦{paidFees.toLocaleString()}</div></div>
                  </div>
                  {fees.length === 0 ? (
                    ...
                    <div className="responsive-table-wrap" style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 14 }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead><tr style={{ borderBottom: '1px solid #334155' }}>{['FEE TYPE', 'AMOUNT', 'DUE DATE', 'STATUS', 'PAID DATE'].map((h) => (<th scope="col" key={h} style={{ padding: 14, textAlign: h === 'FEE TYPE' ? 'left' : 'center', color: '#94a3b8', fontSize: '0.8rem', whiteSpace: 'nowrap' }}>{h}</th>))}</tr></thead>
                        <tbody>
                          {fees.map((f: any) => (
                            <tr key={f.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                              ...
                              <td style={{ padding: 14, textAlign: 'center' }}><span style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, background: f.status === 'paid' ? 'rgba(16,185,129,0.15)' : f.status === 'overdue' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)', color: f.status === 'paid' ? '#10b981' : f.status === 'overdue' ? '#ef4444' : '#f59e0b' }}>{f.status}</span></td>
                              <td style={{ padding: 14, textAlign: 'center', color: '#94a3b8' }}>{f.paid_at ? new Date(f.paid_at).toLocaleDateString() : '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      {homework.map((hw: any) => (
                        <div key={hw.id} style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 12, padding: 20 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <span style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, background: hw.due_date && new Date(hw.due_date) < new Date() ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)', color: hw.due_date && new Date(hw.due_date) < new Date() ? '#ef4444' : '#f59e0b' }}>
                      ...
                      {terms.map((term: any) => {
                        ...
                        return (
                          <div key={term.id} style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid #334155', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                            <div><h4 style={{ margin: '0 0 4px', color: '#e2e8f0' }}>{termLabel}</h4><p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>{term.is_active ? '🟢 Current Term' : '⚪ Past Term'}</p></div>
                            <button onClick={() => handleDownloadReportCard({ label: termLabel.split(' - ')[1] || `Term ${term.id}`, year: term.academic_year || '' })}
                              style={{ padding: '10px', background: '#a855f7', border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem' }}>📥 Download PDF</button>
                          </div>
                        )
                      })}
                    </div>
      ...
      {showPasswordModal && (
        <PasswordChangeModal userInfo={userInfo} userRole="parent" onClose={() => setShowPasswordModal(false)}
          showToast={(msg, type) => { setToast({ msg, type }); setTimeout(() => setToast(null), 3000) }} />
      )}
      {toast && (
        <div role="status" aria-live="polite" style={{ position: 'fixed', bottom: 20, right: 20, padding: '12px 24px', borderRadius: 8, zIndex: 1001, background: toast.type === 'success' ? '#10b981' : '#ef4444', color: 'white', fontWeight: 600, fontSize: '0.9rem', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
          {toast.msg}
        </div>
      )}
    </div>
