...
function Applications({ showToast }: ApplicationsProps) {
  ...
  const fetchData = useMemo(() => (rangeStart: number, rangeEnd: number) => {
    return supabase
      .from('applications')
      .select('id, student_first_name, student_last_name, status, application_number, class_applying_for, student_gender, created_at, student_dob, previous_school, father_name, father_phone, father_email, mother_name, mother_phone, mother_email, address, emergency_contact_name, emergency_contact_phone, medical_notes, how_heard, siblings_enrolled')
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        <button onClick={() => setPage(1)} style={{ padding: '10px 16px', background: '#334155', border: 'none', borderRadius: '10px', color: '#f8fafc', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '600' }}>
