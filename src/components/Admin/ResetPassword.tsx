...
const ROLES = [
  ...
  { key: 'teacher', label: 'Teacher', table: 'teachers', idField: 'id', searchFields: ['first_name', 'last_name', 'staff_id', 'login_id'], nameField: ['first_name', 'last_name'] as const },
  { key: 'parent', label: 'Parent', table: 'parents', idField: 'id', searchFields: ['first_name', 'last_name', 'parent_id', 'login_id'], nameField: ['first_name', 'last_name'] as const },
  { key: 'admin', label: 'Admin', table: 'profiles', idField: 'id', searchFields: ['first_name', 'last_name', 'login_id'], nameField: ['first_name', 'last_name'] as const },
]
...
export default function ResetPassword({ showToast }: ResetPasswordProps) {
  const [role, setRole] = useState<RoleConfig>(ROLES[0])
  ...
  const handleReset = async (user: any) => {
    ...
    try {
      ...
      const updateData: Record<string, any> = { password: hashed }
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '20px', alignItems: 'end' }}>
        <div>
          ...
          <select value={role.key} onChange={(e) => { setRole(ROLES.find(r => r.key === e.target.value) || ROLES[0]); setUsers([]); setResetResult(null) }}
        ...
        <div style={{ flex: 1, minWidth: '160px' }}>
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e: React.KeyboardEvent) => e.key === 'Enter' && handleSearch()}
      ...
      {users.length > 0 && (
        <div style={{ background: '#1e293b', borderRadius: '8px' }}>
