...
export default function ManageNews({ showToast }: ManageNewsProps) {
  ...
  const fetchNews = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('public_news')
        .select('id, title, excerpt, category, image_url, published_date')
        .order('published_date', { ascending: false })

      if (error) throw error
      setNews(data || [])
    } catch (err) {
      showToast?.('Failed to load news: ' + (err instanceof Error ? err.message : String(err)), 'error')
    } finally {
      setLoading(false)
    }
  }
