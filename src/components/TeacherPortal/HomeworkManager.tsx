...
export default function HomeworkManager({ teacherId, showToast }: HomeworkManagerProps) {
  ...
  const fetchHomeworks = async () => {
    setLoading(true);
    const { data } = await safeQuery(() => supabase.from("homeworks").select("id, title, description, due_date, class_id, created_at").order("created_at", { ascending: false }));
    setHomeworks(data || []);
    setLoading(false);
  };
  ...
  const createHomework = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("homeworks").insert({
        teacher_id: teacherId, class_id: selectedClass, title, description, due_date: dueDate, created_at: new Date().toISOString(),
      });
  ...
  return (
    <div style={{ padding: 30, background: "#0f172a", minHeight: "100vh" }}>
      ...
      <div style={{ background: "rgba(30,41,59,0.5)", border: "1px solid #334155", borderRadius: 14, padding: 24, marginBottom: 30 }}>
        ...
        <form onSubmit={createHomework} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              ...
              <select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} required
                style={{ width: "100%", padding: 12, background: "#0f172a", border: "1px solid #334155", borderRadius: 10, color: "#e2e8f0", outline: "none" }}>
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} placeholder="Detailed instructions..."
              style={{ width: "100%", padding: 12, background: "#0f172a", border: "1px solid #334155", borderRadius: 10, color: "#e2e8f0", outline: "none", resize: "vertical" }} />
          </div>
          <button type="submit" disabled={saving}
            style={{ padding: "12px 24px", background: saving ? "#334155" : "#a855f7", color: saving ? "#94a3b8" : "#fff", border: "none", borderRadius: 10, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer" }}>
      ...
      {loading ? (
        <div style={{ padding: '20px 0' }}><CardSkeleton lines={2} /><CardSkeleton lines={3} /><CardSkeleton lines={2} /></div>
      ) : homeworks.length === 0 ? (
